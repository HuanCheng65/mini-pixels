...
int main() {
    ...
    while (true) {
        ...
        if (lowerInputStr == "exit" || lowerInputStr == "quit" ||
            lowerInputStr == "-q") {
            std::cout << "Bye." << std::endl;
            break;
        }
        ...
        if (command == "LOAD") {
            ...
            desc.add_options()("help,h", "show this help message and exit")(
                "origin,o", bpo::value<std::string>()->required(),
                "specify the path of original data files")(
                "target,t", bpo::value<std::string>()->required(),
                "specify the path of target data files")(
                "schema,s", bpo::value<std::string>()->required(),
                "specify the schema of pixels")(
                "row_num,n", bpo::value<int>()->required(),
                "specify the max number of rows to write in a file")(
            ...
            try {
                bpo::store(
                    bpo::parse_command_line(argv.size(), argv.data(), desc),
                    vm);
                if (vm.count("help")) {
