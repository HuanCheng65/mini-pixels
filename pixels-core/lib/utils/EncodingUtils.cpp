...
void EncodingUtils::unrolledUnPackBytes(
    long *buffer, int offset, int len, const std::shared_ptr<ByteBuffer> &input,
    int numBytes) {
    int numHops = 8;
    int remainder = len % numHops;
    int endOffset = offset + len;
...
void EncodingUtils::readLongBE(const std::shared_ptr<ByteBuffer> &input,
                               long *buffer, int start, int numHops,
                               int numBytes) {
...
void EncodingUtils::readRemainingLongs(const std::shared_ptr<ByteBuffer> &input,
                                       long *buffer, int offset, int remainder,
                                       int numBytes) {
...
void EncodingUtils::unrolledBitPack1(long *input, int offset, int len,
    ...
    byte val = 0;
    for (int i = offset; i < endUnroll; i += numHops) {
        val = (byte)(val | ((input[i] & 1) << 7) | ((input[i + 1] & 1) << 6) |
...
void EncodingUtils::unrolledBitPack2(long *input, int offset, int len,
    ...
    byte val = 0;
    for (int i = offset; i < endUnroll; i += numHops) {
        val = (byte)(val | ((input[i] & 3) << 6) | ((input[i + 1] & 3) << 4) |
...
void EncodingUtils::unrolledBitPack4(long *input, int offset, int len,
    ...
    byte val = 0;
    for (int i = offset; i < endUnroll; i += numHops) {
        val = (byte)(val | ((input[i] & 15) << 4) | (input[i + 1] & 15));
        output->put(val);
        val = 0;
    }
...
void EncodingUtils::unrolledBitPack8(long *input, int offset, int len,
                                     std::shared_ptr<ByteBuffer> output) {
...
void EncodingUtils::writeLongLE(std::shared_ptr<ByteBuffer> output,
                                long value) {
    writeBuffer[0] = (byte)((value) & 0xff);
    writeBuffer[1] = (byte)((value >> 8) & 0xff);
    writeBuffer[2] = (byte)((value >> 16) & 0xff);
 ...
 */
void EncodingUtils::writeLongBE(std::shared_ptr<ByteBuffer> output, long *input,
                                int offset, int numHops, int numBytes) {
    switch (numBytes) {
    case 1:
...
void EncodingUtils::writeLongBE2(std::shared_ptr<ByteBuffer> output, long val,
                                 int wbOffset) {
    writeBuffer[wbOffset] = (byte)(((unsigned long)val) >> 8);
    writeBuffer[wbOffset + 1] = (byte)(val);
}

void EncodingUtils::writeLongBE3(std::shared_ptr<ByteBuffer> output, long val,
                                 int wbOffset) {
...
void EncodingUtils::writeLongBE4(std::shared_ptr<ByteBuffer> output, long val,
                                 int wbOffset) {
...
void EncodingUtils::writeLongBE5(std::shared_ptr<ByteBuffer> output, long val,
                                 int wbOffset) {
...
void EncodingUtils::writeLongBE6(std::shared_ptr<ByteBuffer> output, long val,
                                 int wbOffset) {
...
void EncodingUtils::writeLongBE7(std::shared_ptr<ByteBuffer> output, long val,
                                 int wbOffset) {
...
void EncodingUtils::writeLongBE8(std::shared_ptr<ByteBuffer> output, long val,
                                 int wbOffset) {
...
void EncodingUtils::writeRemainingLongs(std::shared_ptr<ByteBuffer> output,
                                        int offset, long *input, int remainder,
                                        int numBytes) {
...
void EncodingUtils::writeShortLE(std::shared_ptr<ByteBuffer> output,
                                 int16_t value) {
    writeBuffer[0] = (byte)((value) & 0xff);
    writeBuffer[1] = (byte)((value >> 8) & 0xff);
    output->putBytes(writeBuffer, 2);
}

void EncodingUtils::writeShortBE(std::shared_ptr<ByteBuffer> output,
                                 int16_t value) {
