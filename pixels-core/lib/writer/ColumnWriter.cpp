...
const int ColumnWriter::ISNULL_ALIGNMENT =
    std::stoi(ConfigFactory::Instance().getProperty("isnull.bitmap.alignment"));
const std::vector<uint8_t>
    ColumnWriter::ISNULL_PADDING_BUFFER(ColumnWriter::ISNULL_ALIGNMENT, 0);
...
int ColumnWriter::getColumnChunkSize() const {
    return static_cast<int>(outputStream->getWritePos() -
                            outputStream->getReadPos());
}
...
std::shared_ptr<pixels::proto::ColumnChunkIndex>
ColumnWriter::getColumnChunkIndexPtr() {
    return columnChunkIndex;
}
...
void ColumnWriter::flush() {
    ...
    if (ISNULL_ALIGNMENT != 0 && isNullOffset % ISNULL_ALIGNMENT != 0) {
        ...
        outputStream->putBytes(
            const_cast<uint8_t *>(ISNULL_PADDING_BUFFER.data()), alignBytes);
        isNullOffset += alignBytes;
    }
    columnChunkIndex->set_isnulloffset(isNullOffset);
    outputStream->putBytes(
        isNullStream->getPointer() + isNullStream->getReadPos(),
...
void ColumnWriter::newPixel() {
    if (hasNull) {
        auto compacted =
            BitUtils::bitWiseCompact(isNull, curPixelIsNullIndex, byteOrder);
        isNullStream->putBytes(const_cast<uint8_t *>(compacted.data()),
                               compacted.size());
...

{
    ...
    columnChunkIndex->set_littleendian(byteOrder ==
                                       ByteOrder::PIXELS_LITTLE_ENDIAN);
