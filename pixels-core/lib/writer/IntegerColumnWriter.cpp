...
int IntegerColumnWriter::write(std::shared_ptr<ColumnVector> vector, int size) {
    ...
    // one doing this pre-calculation to eliminate branch prediction inside the
    // for loop
...
void IntegerColumnWriter::writeCurPartLong(
...
bool IntegerColumnWriter::decideNullsPadding(
    std::shared_ptr<PixelsWriterOption> writerOption) {
...
void IntegerColumnWriter::newPixel() {
    ...
    if (runlengthEncoding) {
        ...
        if (isLong) {
            curVecPartitionBuffer = std::make_shared<ByteBuffer>(
                curPixelVectorIndex * sizeof(long));
...
IntegerColumnWriter::getColumnChunkEncoding() const {
