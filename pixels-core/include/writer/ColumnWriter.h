...
class ColumnWriter {
  public:
    ...
    ColumnWriter(std::shared_ptr<TypeDescription> type,
     ...
     *
     */
    virtual int write(std::shared_ptr<ColumnVector> columnVector,
                      int length) = 0;

    virtual std::vector<uint8_t> getColumnChunkContent() const;
    virtual int getColumnChunkSize() const;
    virtual bool
    decideNullsPadding(std::shared_ptr<PixelsWriterOption> writerOption) = 0;
    virtual pixels::proto::ColumnChunkIndex getColumnChunkIndex();
    virtual std::shared_ptr<pixels::proto::ColumnChunkIndex>
